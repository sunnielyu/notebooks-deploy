...
function showString(req,res){
  let id = req.body;
  let baseHttp = id.http,
  wsHttp = id.ws,
  folderPath = id.path,
  notebookName = id.notebookName,
  destroyKernel = id.destroyKernel;

  let gatewayUrl = process.env.BASE_GATEWAY_HTTP_URL || id.http;
  let gatewayWsUrl = process.env.BASE_GATEWAY_WS_URL ||  id.ws;
  let demoSrc1 =  '%cd ' + id.path;
  let demoSrc2 = createCode(id.notebookName,id.number);
    ...
    jupyter.Kernel.getSpecs({
      ...
      jupyter.Kernel.startNew({
        ...
        future.onDone = () => {
          ...
          if (destroyKernel){ // if true then destoy kernel, otherwize leave it
...
function getAllPost(req,res){
  return showString(req,res).then( (data) => {
    res.json({Data: data || []});
  }).catch((err) => {
    res.status(500).send(err);
  });
